...
class OrderCRUD(Creatable, Retrievable, Deletable):
    ...
    async def get_all(
        self, pagination: PaginationParams = None, filter: OrderFilter = None
    ) -> list[models.Order] | None:
        return await self._get_all(
            and_(
                (self.__class__.model.status == filter.status)
                if filter.status is not None
                else True,
                (self.__class__.model.created_at >= filter.created_after)
