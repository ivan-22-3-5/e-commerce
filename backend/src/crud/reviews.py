...
class ReviewCRUD(Creatable, Retrievable, Deletable, Updatable):
    ...
    async def create_review(self, review_data: ReviewIn, user_id: int, product_id: int) -> models.Review:
    ...
    async def update_review_by_id(self, review_id: int, review_update_data: ReviewUpdate) -> models.Review | None:
    ...
    async def calculate_average_rating_for_product(self, product_id: int) -> float:
        query = (
            select(func.coalesce(func.avg(self.model.rating), 0.0).label("average_rating"))
            .where(self.model.product_id == product_id)
        )
        result = await self.db.execute(query)
