...
async def _handle_user_tokens(
    user_id: int, response: Response, token_service: TokenService
):
...
async def google_callback(
    ...
    if (
        user := await user_service.get_user_by_identity_provider_id(google_user.id)
    ) is None:
...
async def login(
    ...
    if not (user and verify_password(user_credentials.password, user.password)):
        raise InvalidCredentialsError(
            "No account with the given email exists or the password is wrong"
        )
...
@router.post(
    "/password-recovery/{email}", status_code=status.HTTP_200_OK, response_model=Message
)
async def recover_password(
    email: EmailStr, user_service: UserServiceDep, token_service: TokenServiceDep
):
    ...
    send_password_recovery_email.delay(
        username=user.name, link=recovery_token, email_address=email
    )
...
async def reset_password(
    ...
    if (
        await token_service.is_recovery_token_valid(user_id, new_password.token)
    ) is False:
