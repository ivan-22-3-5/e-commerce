...
reviews_router_individual = APIRouter(
    prefix="/reviews", tags=["Reviews (Individual Operations)"]
)
...
async def create_product_review_endpoint(
    ...
    try:
        review = await review_service.create_review(
            review_data=review_in, user_id=current_user.id, product_id=product_id
        )
        return review
...
@reviews_router_individual.put(
    "/{review_id}", response_model=ReviewOut, status_code=status.HTTP_200_OK
)
...
@reviews_router_individual.delete(
    "/{review_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_a_review_endpoint(
    review_id: int, current_user: CurrentUserDep, review_service: ReviewServiceDep
):
    try:
