...
async def get_products(
    product_service: ProductServiceDep, pagination: PaginationParams = Depends()
):
...
async def search_products(
    ...
    return await product_service.search_products(
        q, categories=categories, pagination=pagination
    )
...
async def update_product(
    product_id: int, product_update: ProductUpdate, product_service: ProductServiceDep
):
    return await product_service.update_product(product_id, product_update)


@router.delete(
    "/{product_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[AdminRole]
)
...
async def add_product_image(
    product_id: int, file: UploadFile, product_service: ProductServiceDep
):
...
async def change_product_images(
    product_id: int, images: list[str], product_service: ProductServiceDep
):
