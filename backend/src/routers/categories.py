...
@router.delete(
    "/{category_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[AdminRole]
)
...
async def link_category_to_product(
    category_id: int, product_id: int, category_service: CategoryServiceDep
):
    category_service.link_category_to_product(category_id, product_id)

    return Message(
        message=f"Category {category_id} has been successfully linked to product with id {product_id}"
    )
