...
from src.crud import (
    CartItemCRUD, ProductCRUD, CategoryCRUD, OrderCRUD,
    RefreshTokenCRUD, RecoveryTokenCRUD, ReviewCRUD
)
...
async def get_google_user_info(access_token: GoogleAccessTokenDep, http: HTTPClientDep) -> GoogleUserInfo:
    async with http.get(settings.GOOGLE_USER_INFO_URL, # Changed to GET as per common practice for userinfo
...
async def get_current_user(token: TokenDep, user_service: UserServiceDep) -> models.User:
    user_id = get_user_id_from_jwt(token)
    user = await user_service.get_user_by_id(user_id)
    if user is None: # Should be handled by get_user_id_from_jwt or user_service.get_user_by_id
        raise Exception("User not found, this should not happen if token is valid") # Or a specific exception
    return user
