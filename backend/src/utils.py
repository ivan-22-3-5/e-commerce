...
def create_jwt_token(*, user_id: int, expires_in: timedelta):
    data_to_encode = {"sub": str(user_id), "exp": datetime.now(UTC) + expires_in}
    return jwt.encode(
        data_to_encode, settings.TOKEN_SECRET_KEY, algorithm=settings.ALGORITHM
    )
    ...
    try:
        ...
        if user_id is None:
            raise InvalidTokenError(
                "Could not validate the token", {"WWW-Authenticate": "Bearer {}"}
            )
...
def generate_confirmation_code():
    return random.randint(
        rules.CONFIRMATION_CODE_LOWER_BOUND, rules.CONFIRMATION_CODE_UPPER_BOUND
    )
