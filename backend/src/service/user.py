...
class UserService:
    ...
    async def register_user(self, user_data: UserIn) -> User:
        confirmation_code_from_redis = await self.redis.get(
            f"confirmation_code:{user_data.email}"
        )
        ...
        if user_data.confirmation_code == 999999:
            ...
            raise InvalidConfirmationCodeError(
                "Confirmation code not found or expired."
            )
        ...
        created_user = await self.user_crud.create(
            User(**user_data.model_dump(exclude={"confirmation_code"}))
        )
        return created_user
    ...
    async def register_user_using_google(self, user_info: GoogleUserInfo) -> User:
        ...
        if not user_info.email_verified:
            raise EmailNotConfirmedError(
                "Cannot register user with unverified Google email."
            )
    ...
    async def get_user_by_identity_provider_id(
