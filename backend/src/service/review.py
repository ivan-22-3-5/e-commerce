...
from src.schemas.review import ReviewIn, ReviewUpdate, ReviewOut
...
class ReviewService:
    ...
    async def _update_product_rating(self, product_id: int):
        average_rating = await self.review_crud.calculate_average_rating_for_product(product_id)
    ...
    async def create_review(
        ...
        new_review = await self.review_crud.create_review(review_data, user_id, product_id)
        await self._update_product_rating(product_id)
        await self.db.refresh(new_review, attribute_names=['user'])  # Ensure user is loaded for ReviewOut
        return new_review
    ...
    async def delete_review(self, review_id: int, current_user_id: int, is_admin: bool):
        review_to_delete = await self.review_crud.get(review_id, on_not_found='return-none')
        ...
        if not is_admin and review_to_delete.user_id != current_user_id:
            raise NotEnoughRightsError("You do not have permission to delete this review.")
    ...
    async def update_my_review(
        ...
        review_to_update = await self.review_crud.get(review_id, on_not_found='return-none')

        if not review_to_update:
            raise ResourceDoesNotExistError(f"Review with ID {review_id} not found.")

        if review_to_update.user_id != current_user_id:
            raise NotEnoughRightsError("You do not have permission to update this review.")

        updated_review = await self.review_crud.update_review_by_id(review_id, review_update_data)

        if not updated_review:
            raise ResourceDoesNotExistError(f"Failed to update review with ID {review_id}.")  # Should not happen

        rating_changed = review_update_data.rating is not None and review_update_data.rating != review_to_update.rating
        if rating_changed:
